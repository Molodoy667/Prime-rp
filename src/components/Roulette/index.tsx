...
export default function Roulette(){
  ...
  function rouletteSound(){try{const AudioContextClass=window.AudioContext||((window as unknown as {webkitAudioContext?:typeof AudioContext}).webkitAudioContext);if(!AudioContextClass)return;const context=new AudioContextClass();const master=context.createGain();master.gain.value=.07;master.connect(context.destination);for(let index=0;index<25;index+=1){const oscillator=context.createOscillator();const gain=context.createGain();const start=context.currentTime+index*.22;oscillator.type=index%3?'triangle':'square';oscillator.frequency.setValueAtTime(130+index*7,start);gain.gain.setValueAtTime(.001,start);gain.gain.exponentialRampToValueAtTime(.22,start+.018);gain.gain.exponentialRampToValueAtTime(.001,start+.11);oscillator.connect(gain).connect(master);oscillator.start(start);oscillator.stop(start+.13)}const finish=context.currentTime+5.55;[392,523,659].forEach((frequency,index)=>{const oscillator=context.createOscillator();const gain=context.createGain();oscillator.type='sine';oscillator.frequency.value=frequency;gain.gain.setValueAtTime(.001,finish);gain.gain.exponentialRampToValueAtTime(.25,finish+.04);gain.gain.exponentialRampToValueAtTime(.001,finish+.7);oscillator.connect(gain).connect(master);oscillator.start(finish+index*.04);oscillator.stop(finish+.75)})}catch{/* sound is optional and may be blocked by the browser */}}
  async function spin(count=spinCount,mode:'free'|'paid'='free'){if(!player||rolling)return;setRolling(true);setConfirmCount(null);setError('');rouletteSound();try{const response=await fetch('/api/roulette',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'spin',playerId:player.id,count,mode})});const data=await response.json();if(!response.ok)throw new Error(data.error||'Не вдалося обертати рулетку');const results=(data.results||[{...data.prize,winId:data.winId,sellPrice:data.sellPrice}]).map((item:WinItem&{prize?:Prize})=>({...item.prize||item,winId:item.winId||item.id,sellPrice:item.sellPrice,status:'pending' as const,createdAt:new Date().toISOString()}));setFreeSpins(Number(data.freeSpins)||0);setBalance(Number(data.balance)||0);setSelected(results[0] as Prize);setTimeout(()=>{setPendingRewards(results);setWins(current=>[...results,...current].slice(0,30));setHistory(current=>[...results,...current].slice(0,30));setRolling(false)},quickMode?1400:6200)}catch(value){setError(value instanceof Error?value.message:'Не вдалося обертати рулетку');setRolling(false)}}
