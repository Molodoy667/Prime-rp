...
export default function App() {
  ...
  const [launcherUrl, setLauncherUrl] = useState(siteConfig.launcherUrl); const [siteSettings, setSiteSettings] = useState<Record<string, string>>({}); const [overlay, setOverlay] = useState<OverlayContent | null>(null);
  useEffect(() => { const controller = new AbortController(); getNews(controller.signal).then(setNews).catch(error => { if (error.name !== 'AbortError') setNewsError('Новини тимчасово недоступні.'); }); getSiteSettings(controller.signal).then(settings => { setSiteSettings(settings); if (settings.download_url) setLauncherUrl(settings.download_url); }).catch(() => undefined); return () => controller.abort(); }, []);
  return <MotionConfig reducedMotion="user"><OverlayContext.Provider value={setOverlay}><a href="#main" className="skip-link">Перейти до вмісту</a><Header /><main id="main"><Hero servers={servers} /><ServerStatus servers={servers} error={serverError} loading={serverLoading} onRefresh={refresh} /><Features /><Careers /><Vehicles /><Economy /><Suspense fallback={<div className="map-placeholder" />}><UkraineMap /></Suspense><HowToStart launcherUrl={launcherUrl} /><News items={news} error={newsError} /><Gallery /><FinalCTA /></main><Footer settings={siteSettings}/>{overlay && <Suspense fallback={null}><Overlay content={overlay} onClose={() => setOverlay(null)} /></Suspense>}</OverlayContext.Provider></MotionConfig>;
}
